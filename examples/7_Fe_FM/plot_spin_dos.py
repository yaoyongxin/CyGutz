            ...
            for isy in range(psi_sksn.shape[2]):
                for n in range(bnd_ne[k, 1], bnd_ne[k, 2]+1):
                    for a in range(psi_sksn.shape[-1]):
                        psi_skn_w[isp, k, n-1, a] += (
                                psi_sksn[isp, k, isy, n - bnd_ne[k, 1], a]*
                                np.conj(psi_sksn[isp, k, isy, n -
                                bnd_ne[k, 1], a])).real
...
if __name__ == "__main__":
    ...
    bnd_ne = f["/BND_NE"][...]
    # expansion coefficients <\psi | f>, (spin, k-points, sym_ops, bands,
    # orbitals), band index shifted by ne(:,1)
    psi_sksn = f["/BND_VK"][...]
    f.close()
    ...
    f["/energies"] = energies
    f["/dos_tot_dn"] = dos_t[0]
    f["/dos_tot_up"] = dos_t[1]
    f["/dos_f_dn"] = dos_f[0]
    f["/dos_f_up"] = dos_f[1]
    f.close()
    ...
    plt.figure(0)
    plt.fill_between(
        energies, 0, dos_t[0], facecolor='grey', alpha=0.5, label='tot-dn')
    plt.fill_between(
        energies, 0, -dos_t[1], facecolor='grey', alpha=0.5, label='tot-up')
    plt.plot(energies, dos_t[0], color='grey', label='tot-dn')
    plt.plot(energies, -dos_t[1], color='grey', label='tot-up')
    plt.plot(energies, dos_f[0], color='red', label='Fe-3d-dn')
    plt.plot(energies, -dos_f[1], color='blue', label='Fe-3d-up')
    plt.axvline(x=e_fermi, ls='--')
    plt.title("Density of States with $Fe-3d$-component")
    plt.xlabel("E (eV)")
    plt.ylabel("DOS (states/f.u.)")
    plt.legend()
    plt.savefig("FeDOS.png")
#    plt.show()
