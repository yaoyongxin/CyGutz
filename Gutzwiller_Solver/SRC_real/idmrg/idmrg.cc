...
int main(int argc, char* argv[])
{
    ...
    while (std::getline(infile, line))
    {
        ...
        ampo += eval, op1, (m1+1)/2, op2, (m2+1)/2;
    }
    infile.close();
    ...
    std::vector<std::vector<double>> dm(2*N, std::vector<double>(2*N, 0.));
    ...
    for (int j=1; j <= N; ++j)
    {
        ...
        Op.set(s(3), sp(2), 1);
        psi.position(j);
        auto res = (dag(prime(psi.A(j), Site))*Op*psi.A(j)).real();
        dm[2*j-2][2*j-1] = res;
        dm[2*j-1][2*j-2] = res;
    }
    ...
    for (int i=0; i < 2*N; ++i)
    {
        for (int j=0; j < 2*N; ++j)
        {
            oufile << std::setw(20) << std::setprecision(12) << dm[i][j]; 
        }
        oufile << std::endl;
    }
    oufile.close();
