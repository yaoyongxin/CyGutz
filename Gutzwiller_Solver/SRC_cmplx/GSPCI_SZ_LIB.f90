...
subroutine setup_ncab_op()
    ...
    use gutil
    implicit none
    integer ia1,ia2,isum,istate,ival,itmp1,isgn,nfs_l,i1,i2,ibs
...
subroutine setup_nfab_op()
    ...
    use gutil
    implicit none
    integer ia1,ia2,isum,istate,ival,itmp2,isgn,nfs_l,i1,i2,ibs
    integer jdx(dmem%nstates)
    real(q) aval(dmem%nstates)
...
subroutine calc_reduced_rho_nblk(rho,n,ival)
    ...
    do i=dmem%idx(ival),dmem%idx(ival+1)-1
        ...
        do j=dmem%idx(ival),i
            ...
            rho(i_,j_)=zdotc(ndim,dmem%v(dmem%i_phi(j)%imin),1,&
