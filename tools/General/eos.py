...
def Murnaghan(parameters,vol):
  ...
  BP = parameters[2]
  V0 = parameters[3]
  return E0 + B0*vol/BP*(((V0/vol)**BP)/(BP-1)+1) - V0*B0/(BP-1.0)
...
def get_ev_fit(v, e):
  ...
  Based on http://gilgamesh.cheme.cmu.edu/doc/software/jacapo/appendices/appendix-eos.html
  '''
...
def eos_spline(v, e, tol):
  '''
  Get volume, energy, pressure, and bulk modulus using spline.
  '''
  ...
  ph = [-s.derivatives(i)[1]*convP2 for i in vh]
  bh = [s.derivatives(i)[2]*vh[i]*convP2 for i in vh]
  return vh,eh,ph,bh
