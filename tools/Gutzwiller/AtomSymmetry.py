...
def get_Lie_parameter_3D(g, plus2pi=False):
    ...
    if fix <= -1.0:
        assert(np.abs((np.trace(g) - 1.0) / 2.0 - (-1.0)) < 0.000000001)
        fix = -1.0
    if fix >= 1.0:
        assert(np.abs((np.trace(g) - 1.0) / 2.0 - (1.0)) < 0.000000001)
        fix = 1.0
...
def get_plus2pi(Jz):
    '''
    Decide whether one needs to double the number of Lie parameters due to half-integer of J.
    '''
...
def check_Lie_parameters_3D(Rotations, Lie_parameters):
    '''
    Compare the 3D rotation matrices with the one generated by Lie parameters and the generator J.
    '''
    J = get_3D_rotation_generator()
...
def get_ired_representation(representation):
    ...
    from matrix_util import get_matrices_bk_list, sym_array
    from scipy.linalg import block_diag
    check = 'NO'
...
def get_chi_repr(list_chi, list_repr, list_eigen_space):
    ...
    for i, product in enumerate(chi_product):
        ...
        list_chi_space.append(chi_space)
