...
def get_matrix_Sp_CH_spin_fast(l_list):
    ...
    '''
    num_lm = np.sum(2 * np.array(l_list) + 1)
    Sp_sub = get_matrix_Lp_CH(0.5)
    Sp_list = [Sp_sub for i in range(num_lm)]
    from scipy.linalg import block_diag
    return block_diag(*Sp_list)


def get_S_vector_CH_spin_fast(l_list):
    '''
    Get S-vector in CH basis with spin-fast-index.
    '''
    Sz = get_matrix_Sz_CH_orbital_fast(l_list)
    Sz = trans_orbital_fast_to_spin_fast(Sz)
    Sp = get_matrix_Sp_CH_spin_fast(l_list)
    _, Sx, Sy = get_other_op(Sz, Sp)
    return [Sx, Sy, Sz]


def get_L_vector_CH_spin_fast(l_list):
    '''
    Get L-vector in CH basis with spin-fast-index.
    '''
    Lz, Lp = get_matrix_Lzp_CH_orbital_fast(l_list)
    Lz = trans_orbital_fast_to_spin_fast(Lz)
    Lp = trans_orbital_fast_to_spin_fast(Lp)
    _, Lx, Ly = get_other_op(Lz, Lp)
    return [Lx, Ly, Lz]


def get_trans_JJ_to_CH_spin_fast(l_list):
    '''
    Get the unitary transformation from JJ to CH_spin_fast basis.
    '''
    from matrix_util import Rtrans_U_supdn_to_spin_fast_sdnup, \
            trans_JJ_to_CH_sup_sdn
    U_list = []
    for l in l_list:
        U = trans_JJ_to_CH_sup_sdn(l)
        U_list.append(Rtrans_U_supdn_to_spin_fast_sdnup(U))
...
def get_S_vector_JJ(l_list):
    ...
    '''
    S_vec = get_S_vector_CH_spin_fast(l_list)
    U = get_trans_JJ_to_CH_spin_fast(l_list)
    for i, _S in enumerate(S_vec):
        S_vec[i] = np.dot(U, np.dot(_S, np.conj(U.T)))
    return S_vec
...
def get_L_vector_JJ(l_list):
    ...
    '''
    L_vec = get_L_vector_CH_spin_fast(l_list)
    U = get_trans_JJ_to_CH_spin_fast(l_list)
...
def get_S_vector(l_list, soc):
    ...
    if 'y' in soc:
        ...
        return get_S_vector_CH_spin_fast(l_list)


def get_L_vector(l_list, soc):
    '''
    Dispatcher.
    '''
    if 'y' in soc:
        ...
        return get_L_vector_CH_spin_fast(l_list)
