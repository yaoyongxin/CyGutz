...
def calc_save_atomic_states(imp=1, num_ev=100):
  ...
  vals, n_labels, j_labels, chi_labels, multiplet_degeneracies \
      = get_local_histogram(imp, f, op_list = ["N"], num_ev = num_ev)
  f.close()
...
def plot_atomic_states(imp=1, num_label = 5):
  ...
  print " sum_weights = ",f[base+"/sum_weights"][...]
  f.close()
  ...
  labels = ['N=%1d,D=%1d'%(int(n+0.1),int(deg+0.1)) for n,deg in zip(n_labels[:num_label],multiplet_degeneracies[:num_label])]
...
if __name__=="__main__":
  try:
    plot_atomic_states(imp=1, num_label = 10)
  except:
