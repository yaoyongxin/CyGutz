...
def setup_bare_hamiltonian(e_list, U):
    # BNDU_
    ek_list = [[e - U/2., e - U/2.] for e in e_list]
    Uk_list = [numpy.diag([1., 1+0.j]) for e in e_list]
...
if __name__=="__main__":
    ...
    Umet_ga_d = numpy.arange(0.,3.3,0.1)
    etot_list = []
    for U in Umet_ga_d:
        setup_bare_hamiltonian(e_list, U)
        subprocess.call('./CyGutz', shell=True)
        with h5py.File('glog.h5', 'r') as f:
            etot_list.append(f['/E_TB_TOT'][0]+U/2.)
    print(numpy.asarray(Umet_ga_d))
    print(numpy.asarray(etot_list))
