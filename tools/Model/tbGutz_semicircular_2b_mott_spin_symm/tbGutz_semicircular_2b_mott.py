...
def setup_gutzwiller(e_list):
    ...
    sig_half = numpy.asarray([[1,2],[3,4]])
    sigma = numpy.zeros((4,4),dtype=numpy.int)
    sigma[::2,::2] = sig_half
    sigma[1::2,1::2] = sig_half
    sigma_list = [sigma]
    U_list = [numpy.identity(max_dim_sorbit*2, dtype = numpy.complex)]
    ...
    type_1atom = [0]; df_list = ["g"]; dim_list = [max_dim_sorbit*2]
...
def setup_bare_hamiltonian(e_list, U):
    # BNDU_
    ek_list = [[e - U/2., 0.] for e in e_list]
    Uk_list = [numpy.diag([1.0, 1+0.j]) for e in e_list]
    gl_interface.write_gutz_bndu(0,ek_list,Uk_list)
    with open('V2AO.INP', 'w') as f:
        f.write("NT=  1 \n")
        f.write("1 1 1 1 {}".format(U))
