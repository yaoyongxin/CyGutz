SUBROUTINE ATPAR (REL,NAT,JATOM,LATOM,cform,ZZ)                            
  ...
  DO l=0,LMAX2     ! 70
     ...
     TRY=-CROSS                            
     IF(TRY.LT.(-0.05d0) .AND. (myrank.EQ.master .OR. fastFilesystem) ) WRITE(6,9) L,TRY,OVLP
     IMAX=JRI(JATOM)                                                   
     DO M=1,IMAX                                                    
        AE(M)=(AE(M)+TRY*A(M))
        BE(M)=(BE(M)+TRY*B(M))                                            
...
!
  ...
  lo_loop: DO l=0,lomax 
     DO jlo=1,ilo(l)
        ...
        if (myrank.EQ.master .OR. fastFilesystem) WRITE(6,800) L,Plo(jlo,l),DPlo(jlo,l),NODEL,NODES,NODE
     ENDDO
