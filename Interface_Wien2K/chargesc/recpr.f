SUBROUTINE RECPR (NWAVE,INDMAX,KXMAX,KYMAX,KZMAX,GMAX)            
  ...
  ABMAX=-999
  if (myrank.EQ.master .OR. fastFilesystem) WRITE(6,*) ' KXMAX,KYMAX,KZMAX',KXMAX,KYMAX,KZMAX
  KMAX(1)=1                                                     
  KMAX(2)=1                                                     
  KMAX(3)=1                                                     
  LL1=KXMAX+1                                                       
...
1    CONTINUE                                                          
  ...
  if (myrank.EQ.master .OR. fastFilesystem) WRITE(6,1010) Nk                                      
  !     reduce dimensions to actual value
  if (myrank.EQ.master .OR. fastFilesystem) write(6,*) 'nwav1,kn',nwav1,nk
  ...
  DO I=1,Nk                                                  
     ...
     CALL STERN(IM,NST,ISTM,TAUP)
     INST(I)=NST                                                    
     if (myrank.EQ.master .OR. fastFilesystem) WRITE(6,1020) I,KZZ(1,I),KZZ(2,I),KZZ(3,I),absk(i),nst             
     DO JJ=1,NST   
  ...
  if (myrank.EQ.master .OR. fastFilesystem) WRITE(6,1030) INDMAX                                  
  nwave1=nwave
  deallocate (absk)
  RETURN                                                            
100 FORMAT(I4)                                                        
