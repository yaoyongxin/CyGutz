SUBROUTINE ANGLE(XMS,THETA,PHI)
  ...
  if (myrank.EQ.master .OR. fastFilesystem) write(6,*)'LATTICE:',lattic, ORTHO, AA, BB, CC, XMS
  IF (ORTHO) THEN
     XA=AA*XMS(1)
     XB=BB*XMS(2)
     XC=CC*XMS(3)
  ...
  ELSE 
	...
	TT=ABS((ALPHA(1)-PI/2.d0)*(ALPHA(2)-PI/2.d0)*(ALPHA(3)-PI/2.d0))
        ...
        if (myrank.EQ.master .OR. fastFilesystem) write(6,*) ' Triclinic implemented, but never tested'
