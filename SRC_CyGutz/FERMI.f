subroutine eord(e,eb,nemax,k,jspin)
  ...
  dimension eb(nemax,k,2),e(nemax*jspin,k)
  do k1=1,k
     ne2=0
     do ispin=1,jspin
        do ne1=1,nemax
     ...
     if(cordeg.lt.0.d0) then
        ...
        do jspin1=1,jspin
           ...
           do while(nn.le.nnn)
              ...
              if(ifcp.eq.1) then
                 if(ifcpt.eq.1) then
                    write(GL%IO,'("k-pnt spin band  energy          old/new occ.    ")')
                    ifcpt=0
                 endif
                 do icp=0,ndeg-1
                    write(GL%IO,50)kk,jspin1,nn,eb(nn+icp,kk,jspin1),weight((nn+icp)+(jspin1-1)*nnn,kk)
                    weight((nn+icp)+(jspin1-1)*nnn,kk) = wecp/dble(ndeg)
                    write(GL%IO,'(f9.6)') weight((nn+icp)+(jspin1-1)*nnn,kk)
                 enddo
              endif
              nn=nn+ndeg
           enddo
...
!
     DO 8 NN=1,NNN   
        ...
        enddo
   8 CONTINUE
     write(GL%IO,*) '  number of occupied bands:',nbmax
     write(GL%IO,*) '  highest energy:',emax
     return
...
!
  write(GL%IO,*) '  FERMI ENERGY AT ',EF                                   
  !     -----------------------------------------------------------------
  !     --  CALCULATE WEIGHTS                                           -
  !     -----------------------------------------------------------------
...
!
  IF(DABS(SUM-RNTOT).GT.TOLMAX) THEN                               
     WRITE (GL%IO,9000) SUM,RNTOT
  ELSE                                                             
     write(GL%IO,*) '  SUM RULE OF TETRAHEDRON INTEGRATION CHECKED '
  END IF
  RETURN                                                           
...
300   CONTINUE                                                         
  ...
  900 WRITE (*,9000) EMIN
      ...
      STOP  'FERMI: EFERMI OUT OF ENERGY RANGE'
