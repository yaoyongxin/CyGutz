...
def get_input_soc_only(imp=1, l=3):
    ...
    if numpy.max(numpy.abs(x_list.imag)) > 1.e-8:
        warnings.warn(' imaginary part detected in ml input!')
    x_list = x_list.real
    return x_list, eshift, [phase1, phase2]


def driver_gs_ml(l=3,valence=5):
    ...
    with open('{}/{}/PrefitKRR_n{}.pkl'.format(dpath, subd,valence), \
                'rb') as f:
        kr = pickle.load(f)
    ...
    with open('{}/{}/PrefitENE_n{}.pkl'.format(dpath, subd,valence), \
                'rb') as f:
        kr = pickle.load(f)
