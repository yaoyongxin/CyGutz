subroutine abcd(l,jatom,pei,pi12lo,pe12lo,is,jlo,lapw)
!     abc calculates the cofficients a,b,c of the lo   
  ...
  logical lapw,rlo
!---------------------------------------------------------------------  
!  OUTPUT: alo in common block /loabc/
!print*, 'l,jlo,lapw,rlo',l,jlo,lapw,rlo
  do irf=1,nrf
...
!      print*,l,is,irf
     ...
     clo=xac*(xac+2.0D0*pi12lo)+xbc*(xbc*pei+2.0D0*pe12lo)+1.0D0  
     clo=1.0D0/sqrt(clo)
     if (clo.gt.2.0D2) clo=2.0d2
     alo(l,is,jlo,1)=clo*xac
     alo(l,is,jlo,2)=clo*xbc
     alo(l,is,jlo,irf)=clo
     if (myrank.EQ.master .OR. fastFilesystem) write (6,10) l,alo(l,is,jlo,1),alo(l,is,jlo,2), alo(l,is,jlo,irf)
  else
     if (jlo.eq.1) then
	alonorm=sqrt(1.d0+(P(l,is,1)/P(l,is,2))**2*PEI)
     ...
     if (myrank.EQ.master .OR. fastFilesystem) write (6,10)l,(alo(l,is,jlo,jrf),jrf=1,nrf)
  end if
