...
!---------------------------------------------------------------------
	...
	XC=CC*XMS(3)
      ...
      ELSE 
	IF ((ABS(ALPHA(3)-PI/2.d0).GT.1.D-4).and.(ABS(ALPHA(2)-PI/2.d0).LT.1.D-4).and. &
            (ABS(ALPHA(1)-PI/2.d0).LT.1.D-4)) THEN
        XA=XMS(1)*AA*SIN(ALPHA(3))
        XB=XMS(1)*AA*COS(ALPHA(3))+BB*XMS(2)
        XC=CC*XMS(3)

        ELSE IF ((ABS(ALPHA(2)-PI/2.d0).GT.1.D-4).and.(ABS(ALPHA(1)-PI/2.d0).LT.1.D-4) &
            .and.(ABS(ALPHA(3)-PI/2.d0).LT.1.D-4)) THEN
