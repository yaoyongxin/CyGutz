SUBROUTINE RECFIL(FNAME1,GMAX,RCFILE,KXMAX,KYMAX,KZMAX,NWAVE)
  ...
  if (RCFILE.eq.'FILE') then
     ...
     if(itape.eq.0) THEN
        if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) 'reading recprlist from file'
        ...
        if (NEWRC .and. (myrank.EQ.master .OR. fastFilesystem/=0)) write (6,*) GMAX1,GMAX,AA1,AA,BB1,BB,CC1,CC, ALPHA1(1),ALPHA(1),ALPHA1(2),ALPHA(2),ALPHA1(3), ALPHA(3),iord1,iord
...
!        generate a new recprlist 
         if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) 'generate new recprlist'
         ...
         call cputim(t2)
         if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) ' time in recpr: ',t2-t1
         if(RCFILE.EQ.'FILE') then
