SUBROUTINE FERMI(nbmax,sumw0)
  ...
  nkpt=nkpt+1
  if (myrank.EQ.master .OR. fastFilesystem/=0) WRITE(6,*) 'nume=', nume, 'nkpt=', nkpt 
...
SUBROUTINE fermi_tetra(nbmax)
  ...
  !
  if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,'(" BZ-integration with TETRA-program.   icor=:",I2  )') icor
  if(cordeg.lt.0.d0 .and. (myrank.EQ.master .OR. fastFilesystem/=0) ) write(6,'(" Equal occupancy of degenerate states, tol=:",E8.1)') -cordeg
...
1234 format(20(10f8.4,/))
  if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*)'call eord...' 
  call eord(e,eb,nemax,k,jspin) 
  if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*)'call dos...' 
  call dos(nemax*jspin,k,e,weight,elecn/2.d0*jspin,d,ef,iw,nw) 
  if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*)'call eweigh...'
     ...
     if(cordeg.lt.0.d0) then
        ...
        do jspin1=1,jspin
           ...
           do while(nn.le.nnn)
              ...
              if(ifcp.eq.1) then
                 if(ifcpt.eq.1) then
                    if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,'("k-pnt spin band  energy          old/new occ.    ")')
                    ifcpt=0
                 endif
                 do icp=0,ndeg-1
                    if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,50)kk,jspin1,nn,eb(nn+icp,kk,jspin1),weight((nn+icp)+(jspin1-1)*nnn,kk)
                    weight((nn+icp)+(jspin1-1)*nnn,kk) = wecp/dble(ndeg)
                    if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,'(f9.6)') weight((nn+icp)+(jspin1-1)*nnn,kk)
                 enddo
              endif
              nn=nn+ndeg
           enddo
...
!
   ...
   8  WEIGH(KK,NN)=WEIGHT(nn,KK) *2.0d0 / jspin                        
     ...
     if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) '  number of occupied bands:',nbmax
     if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) '  highest energy:',emax
     if(ef.gt.emax.and.abs(emax-ebmin(nbmax+1)).gt.1.d-3) then
        if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) 'insulator !'
        if((ef-emax).lt.1.d-4.or.(ef-emax).ge.1.d-4) then
           if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) 'EF-inconsistency corrected'
...
SUBROUTINE DOS(NB,NKP,EB,WGHT,RNTOT,D,EF,W,NWX)                  
  ...
  if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) '  FERMI ENERGY AT ',EF                                   
  !     -----------------------------------------------------------------
  !     --  CALCULATE WEIGHTS                                           -
  !     -----------------------------------------------------------------
  ...
  IF(DABS(SUM-RNTOT).GT.TOLMAX) THEN                               
     if (myrank.EQ.master .OR. fastFilesystem/=0) WRITE (6,9000) SUM,RNTOT
     if (myrank.eq.master) write(21,9001) SUM,RNTOT
  ELSE                                                             
     if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) '  SUM RULE OF TETRAHEDRON INTEGRATION CHECKED '
  END IF
  RETURN                                                           
...
!          write(*,*) ip,DRVAL(W(ONOS),IP),DRVAL(W(ONOS),IP+1)
        ...
        if (myrank.EQ.master .OR. fastFilesystem/=0) write(6,*) 'ILOOP ',ILOOP                                          
        GOTO 1000                                                      
      END IF                                                           
2000  CONTINUE                                                         
      CALL RLSE(ONOS)                                                  
