...
class Struct:
    ...
    def __init__(self, filename, fh_info=sys.stdout):
        ...
        (self.a, self.b, self.c, self.angle[0], self.angle[1], self.angle[2]) = map(
            float, [line[:10], line[10:20], line[20:30], line[30:40], line[40:50], line[50:60]])
        for i in range(3):
            if abs(self.angle[i]) < 1e-5:
                self.angle[i] = 90.
        ...
        for iat in range(self.nat):
            ...
            for mu in range(self.mult[-1] - 1):
                ...
                print >> fh_info, '  iatnr=', self.iatnr[-1],
                self.pos.append(
                    map(float, [line[12:22], line[25:35], line[38:48]]))
            ...
            print >> fh_info, 'aname=', self.aname[
                -1], 'Znuc=', self.Znuc[-1], 'Rmt=', self.rmt[-1]
            rt = []
            for i in range(3):
                line = fh.next()
...
def dmft1(fday, case, fh_info, MPI, ROOT):
    ...
    if not os.path.isfile(name + '.def'):
        ...
        print >> fday, '  stop error: the required input file ' + \
            name + '.def for the next step could not be found!'
...
def AStep(fday, case, name, inp_name, WIEN, para, fh_info):
    if not os.path.isfile(case + '.' + inp_name):
        ...
        print >> fday, '  stop error: the required input file ' + case + \
            '.' + inp_name + ' for the next step could not be found!'
...
if __name__ == '__main__':
    # -------------- Parameters which are set through input files sparams.dat
    # Default values of some parameters
    params = {
        # the double counting scheme, which fixes Edc through n0
        'DCs': 'fixn',
        ...
        # If broyden file are present at the submit time, user has some time
        # to kill the job
        'sleeptime': 2,
        # the charge density precision to stop the LDA+DMFT run
        'cc': 1e-3,
        'ec': 1e-5,    # the energy precision to stop the LDA+DMFT run
        'so': False,   # spin-orbit coupling
        # Reduction of the crystal field splitting, if necessary
        'rCF': None,
        # Average over the last few DMFT self-energies is used for charge.
        # Should be between [0,1]
        'saver': 0.0,
        'StartProg': 'lapw0',   # The program to start with
        'EndProg': ' ',     # Exit after program EndProg
        'openmp': False    # Using openmp instead of mpi
    }
    ...
    if '-omp' in sys.argv:
        p['openmp'] = True
        print 'Using Open-MP instead of MPI.'
    ...
    toclean = glob.glob('*.scf') + glob.glob('Edc.dat') + glob.glob('dmft1.error*') + glob.glob('dmft2.error*') + glob.glob(
        '*.cdos.*') + glob.glob('*.dlt.*') + glob.glob('*.gc1.*') + glob.glob('*.outputdmf1.*') + glob.glob('*.outputdmf2.*')
    for f in toclean:
        os.remove(f)

    # Info files
    fday = open(w2k.case + '.dayfile', 'w')
    print >> fday, 'Calculating ' + w2k.case + ' in ' + \
        os.getcwd() + '\n' + 'on ' + socket.gethostname() + \
        ' with PID ' + str(os.getpid())
    print >> fday, '\n\n'
    ...
    if len(glob.glob('dmft0_info.out')) != 0:
        os.remove('dmft0_info.out')
    if len(glob.glob('dmft1_info.out')) != 0:
        os.remove('dmft1_info.out')
    if len(glob.glob('dmft2_info.out')) != 0:
        os.remove('dmft2_info.out')
    ...
    for icix in cixs:
        ...
        # nuclear charge of each correlated block
        Znuc[icix] = int(struct.aZnuc[atm - 1] + 0.5)
    print >> fh_info, 'Znucs=', Znuc

    # For parallel execution, executable has to be on the current directory
    shutil.copy2(dmfe.ROOT + '/dmft', '.')
    # For parallel execution, executable has to be on the current directory
    shutil.copy2(dmfe.ROOT + '/dmft2', '.')
    # For parallel execution, executable has to be on the current directory
    shutil.copy2(dmfe.ROOT + '/CyGutz', '.')

    print >> fh_info, '--------- Preparing GF calculation ---------'
    cmd = dmfe.ROOT + '/ga_x_dmft.py -d' + \
        para + ' dmft1 >> dmft1_info.out 2>&1'
    print >> fh_info, cmd
    stdin, stdout, stderr = os.popen3(cmd)
    ...
    if p['finish'] > 0:  # Charge self-consistent; 1: One shot
        ...
        if os.path.isfile(w2k.case + '.clmsum_old'):
            shutil.copy2(w2k.case + '.clmsum_old', w2k.case + '.clmsum')
        if not os.path.isfile(w2k.case + '.clmsum'):
            print 'no ' + w2k.case + '.clmsum(_old) file found, which is necessary for lapw0 !'
            print >> fday, 'no ' + w2k.case + \
                '.clmsum(_old) file found, which is necessary for lapw0 !'
            sys.exit(1)
        ...
        print >> fday, '\n   start' + (' ' * 8) + time.asctime() + ' with ' + p[
            'StartProg'] + ' (' + str(1) + '/' + str(p['riter']) + ' to go)'
        print >> fday, '\n   cycle %s \t%s %s/%s to go\n' % (
            0, time.asctime(), p['finish'], (p['finish']) % p['riter'])

        if p['StartProg'] in 'lapw0':
            lapw0(fday, w2k.case, w2k.WIENROOT, para, fh_info)
            fday.flush()
        if p['StartProg'] in 'lapw0 lapw1':
            lapw1(fday, w2k.case, w2k.WIENROOT, para, fh_info)
            fday.flush()
        if p['StartProg'] in 'lapw0 lapw1 lapwso':
            if p['so']:
                lapwso(fday, w2k.case, w2k.WIENROOT, para, fh_info)
                fday.flush()

    if para:
        ga_convert_processes.convert(p['so'], fh_info)
    ...
    while icycle < abs(p['finish']):
        ...
        # DMFT loop     #
        #####################
        if icycle > 0 or (icycle == 0 and p['StartProg'] in \
                'lapw0 lapw1 lapwso dmft1'):
            dmft1(fday, w2k.case, fh_info, dmfe.MPI, w2k.WIENROOT)
            fday.flush()
            if p['openmp']:
                from create_GOMP_file import create_GOMP_file
                create_GOMP_file()
            elif os.path.isfile("GOMP.INP"):
                os.remove("GOMP.INP")
        if p['EndProg'] in 'dmft1':
            break
        if icycle > 0 or (icycle == 0 and p['StartProg'] in \
            ...
            if p['openmp']:
                ...
                MPI = dmfe.MPI
            CyGutz(fday, w2k.case, fh_info, MPI, w2k.WIENROOT)
            fday.flush()
        if p['EndProg'] in 'CyGutz':
            break
        if os.path.isfile(w2k.case + '.outputdmf1.0'):
            shutil.copy2(w2k.case + '.outputdmf1.0', 'GL_LOG.OUT')

        # No need for the following run, backup file 'WH_Bvec_Svec.INP'.
        if os.path.isfile('WH_Bvec_Svec.INP'):
            os.rename('WH_Bvec_Svec.INP', 'WH_Bvec_Svec.OUT')

        dmft2(fday, w2k.case, fh_info, w2k.WIENROOT, dmfe.MPI)
        fday.flush()
        if p['EndProg'] in 'dmft2':
            break

        lcore(fday, w2k.case, w2k.WIENROOT, fh_info)
        fday.flush()
        scf(fday, w2k.case, w2k.WIENROOT)
        fday.flush()
        scf1(fday, w2k.case, w2k.WIENROOT)
        fday.flush()
        mixer(fday, w2k.case, w2k.WIENROOT, fh_info)
        fday.flush()
        scfm(fday, w2k.case, w2k.WIENROOT)
        fday.flush()
        drho, dene = Diff(fday, w2k.case)
        print >> fh_info, '------- charge step', icycle, 'done ---------'
        fday.flush()
        print 'cc=', drho, '->', p['cc'], 'ec=', dene,\
                '->', p['ec'], 'icycle=', icycle
        if drho < p['cc'] and dene < p['ec'] and icycle > -1:
            break
        ...
        if p['so']:
            lapwso(fday, w2k.case, w2k.WIENROOT, para, fh_info)
        if os.path.exists('WH_RLNEF.OUT'):
            shutil.copy2('WH_RLNEF.OUT', 'WH_RLNEF.INP')
